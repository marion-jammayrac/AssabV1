...
export default class ViewUser extends React.Component {
    ...
    searchUser = () => {
        ...
        const { input_name2 } = this.state;
        console.log(this.state.input_name2);
        let two = '%';
        let joined = two + input_name2 + two;
        console.log(joined);
        db.transaction(tx => {
            tx.executeSql(" SELECT * FROM test50 WHERE nom LIKE '" + joined +"' UNION SELECT * FROM test50 WHERE age_max LIKE " + input_name +" ", [], (tx, results) => {
        ...
        return (
            <>
                ...
                <SafeAreaView style={styles.container}>
                    <View>
                        <Mytextinput
                            placeholder="Enter Structure age"
                            onChangeText={input_name => this.setState({ input_name })}
                            style={{ padding: 10 }}
                        />
                        <Mytextinput
                            placeholder="Enter Structure name"
                            onChangeText={input_name2 => this.setState({ input_name2 })}
                            style={{ padding: 10 }}
                        />
                        <Mybutton
                            title="Search User"
